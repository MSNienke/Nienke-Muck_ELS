...
- Shönere Structure, If Instructions - > switch / case 
- Programm Locks
- Entprellroutine ?
- Angular position Anzege
- Visualization LEDs , sounds 
- variable size
- Bresenham Rirchtung
- Numbers lock Negative 
-....
*/
...
long Error = resolution/2; 

long spindle_posi = 0; //Absolute 
int spindle_angle = 0; //Angle
boolean spindle_dir;
unsigned int spindle_rpm = 0;

long uiInterruptCountHelp=0; // hilfsZÃƒÂ¤her fÃƒÂ¼r the RPM Measurment
long spindel_puls_s=0; // ZÃƒÂ¤her fÃƒÂ¼r the RPM Measurment 
long ulTimeHelp, ulMillisHelp;// time store fÃƒÂ¼r the Measurment
...
void setup() {  
  ...
  attachInterrupt(2, doSpindleZ, CHANGE);
	...
	if (!digitalRead(encoder)) // Charging defalut values ​​and not the memory
...
void print_menue ()
{
	...
	switch (menue){
		...
		case 4:
		GLCD.CursorTo(6,0);
		GLCD.print("Options"); // fixed, Thanks Jürgen
		GLCD.EraseTextLine();
		GLCD.CursorTo(1,1);
		GLCD.print("Dir.:");
		GLCD.EraseTextLine();
...
void doSpindleA(){
 ...
 if (srd.run_state == AUTO)
 {
	 ...
	 new_step_delay = srd.min_delay;
		if(step_count >= srd.decel_start) // termination condition
		{	decl_trigger();
		}
 }
...
{
 configuration.thread_pitch =default_thread_pitch;  // Pitch 2mm equivalent 200, 1.06 corresponds to 106... !!
